      ...
      if (newContent !== null) {
        ...
        if (splitIndex > 0) {
          ...
          if (newTitle.trim() !== '' || newTime.trim() !== '') {
            ...
            // Проверяем, что введенное время соответствует формату "00:00" и находится в диапазоне от 00:00 до 23:59
            if (/^([01]\d|2[0-3]):[0-5]\d$/.test(newTime)) {
              notes[index].time = newTime;
            } else {
              alert('Введите время выполнения заметки в формате "чч:мм" (от 00:00 до 23:59)');
            }
          }
        } else {
